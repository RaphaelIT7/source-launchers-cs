...
namespace MinHook {
    internal sealed class Hook
    {
        ...
        public void Enable(bool enable = true) {
            ...
            long patchSize = Unsafe.SizeOf<JMPCALL_REL>();
            IntPtr patchTarget = trampoline.Target;

            if (trampoline.PatchAbove) {
                patchTarget = patchTarget - Unsafe.SizeOf<JMPCALL_REL>();
                patchSize += Unsafe.SizeOf<JMP_REL_SHORT>();
            }
            ...
            if (enable) {
                ...
                if (trampoline.PatchAbove) {
                    JMP_REL_SHORT shortJmp = new JMP_REL_SHORT(0xEB, (byte)(0 - (Unsafe.SizeOf<JMP_REL_SHORT>() + Marshal.SizeOf(jmp))));
                    Marshal.StructureToPtr(shortJmp, trampoline.Target, false);
                }
            } else {

                if (trampoline.PatchAbove) {
                    Marshal.Copy(trampoline.Backup, 0, patchTarget, Unsafe.SizeOf<JMPCALL_REL>() + Unsafe.SizeOf<JMP_REL_SHORT>());
                } else {
                    Marshal.Copy(trampoline.Backup, 0, patchTarget, Unsafe.SizeOf<JMPCALL_REL>());
                }
            }
