    ...
    internal sealed class Trampoline{
        ...
        public Trampoline(IntPtr target, IntPtr detour, MemorySlot memorySlot ) {
            ...
            if (is64) {
				...
				call_size = Unsafe.SizeOf<CALL_ABS>();
				jmp_size = Unsafe.SizeOf<JMP_ABS>();
				jcc_size = Unsafe.SizeOf<JCC_ABS>();
			} else {
                call = new JMPCALL_REL(0xE8);
                jmp = new JMPCALL_REL(0xE9);
                jcc = new JCC_REL(0);

				call_size = Unsafe.SizeOf<JMPCALL_REL>();
				jmp_size = Unsafe.SizeOf<JMPCALL_REL>();
				jcc_size = Unsafe.SizeOf<JCC_REL>();
            }            
            ...
            do {
                ...
                if(oldPos >= Unsafe.SizeOf<JMPCALL_REL>()) {

                    if (is64) {
                        ((AddressSupport)jmp).SetAddress(oldInst);
                    } else {
                        ((OperandSupport)jmp).SetOperand((uint)((uint)oldInst - ((uint)newInst + jmp_size)));
                    }

                    srcObj = jmp;
                    copySize = (uint)jmp_size;
                    finished = true;
                    ...
                    if (is64) {
                        ...
                        ((OperandSupport)call).SetOperand((uint)dest - (uint)(newInst + call_size));
                    }

                    srcObj = call;
                    copySize = (uint)call_size;
                    ...
                    if((long)Target <= (long)dest && (long)dest < ( (long)Target + Unsafe.SizeOf<JMPCALL_REL>())) {
                        ...
                        if (is64) {
                            ...
                            ((OperandSupport)jmp).SetOperand((uint)((uint)dest - ((uint)newInst + jmp_size)));
                        }
                    ...
                    if((long)Target <= (long)dest && (long)dest < ((long)Target + Unsafe.SizeOf<JMPCALL_REL>())){
                        ...
                        if (is64) {
                            ...
                            ((OperandSupport)jcc).SetOperand((uint)((uint)dest - ((uint)newInst + jcc_size)));
                        }

                        srcObj = jcc;
                        copySize = (uint)jcc_size;
                    }
                ...
                if (srcObj != null) { 
					...
					switch (srcObj) { 
						case CALL_ABS c: Marshal.StructureToPtr(c, data, false); break;
						case JMP_ABS c: Marshal.StructureToPtr(c, data, false); break;
						case JCC_ABS c: Marshal.StructureToPtr(c, data, false); break;
						case JMPCALL_REL c: Marshal.StructureToPtr(c, data, false); break;
						case JCC_REL c: Marshal.StructureToPtr(c, data, false); break;
            ...
            if (oldPos < Unsafe.SizeOf<JMPCALL_REL>()
                ...
                && IsCodePadding((IntPtr)((long)Target + oldPos), (int)(Unsafe.SizeOf<JMPCALL_REL>() - oldPos))) {

                if (oldPos < Unsafe.SizeOf<JMP_REL_SHORT>()
                    && IsCodePadding((IntPtr)((long)Target + oldPos), (int)(Unsafe.SizeOf<JMP_REL_SHORT>() - oldPos))) {

                    throw new Exception("Not enough space for patch");
                }

                if (!IsExecutableAddress( (IntPtr) ( (long)Target - Unsafe.SizeOf<JMPCALL_REL>()))) {
                    throw new Exception("Not enought space for patch");
                }

                if (!IsCodePadding((IntPtr)((long)Target - Unsafe.SizeOf<JMPCALL_REL>()), Unsafe.SizeOf<JMPCALL_REL>())) {
                    throw new Exception("Not enought space for patch");
                }
            ...
            if (is64) {
                ...
                IntPtr data = Marshal.AllocHGlobal(jmp_size);
				switch (jmp) {
					case JMP_ABS c: Marshal.StructureToPtr(c, data, false); break;
					case JMPCALL_REL c: Marshal.StructureToPtr(c, data, false); break;
					default: throw new Exception();
				}
				CopyMemory(Relay, data, jmp_size);
                Marshal.FreeHGlobal(data);              
            }
        ...
        void BackupPrologue() {
            ...
            if (PatchAbove) {
                ...
                Marshal.Copy((IntPtr)((long)Target - Unsafe.SizeOf<JMPCALL_REL>()), 
                    Backup, 0, Unsafe.SizeOf<JMPCALL_REL>() + Unsafe.SizeOf<JMP_REL_SHORT>());

            } else {
                Marshal.Copy(Target, Backup, 0, Unsafe.SizeOf<JMPCALL_REL>());
            }
        }
