...
namespace MinHook {
    public sealed class HookEngine : IDisposable {
		...
		public Func CreateAndEnableHook<Func>(string dll, string function, Func detour) where Func : Delegate {
			Func original = CreateHook(dll, function, detour);
			EnableHook(original);
			return original;
		}

		public Func CreateAndEnableHook<Func>(IntPtr location, Func detour) where Func : Delegate {
			Func original = CreateHook(location, detour);
			EnableHook(original);
			return original;
		}
